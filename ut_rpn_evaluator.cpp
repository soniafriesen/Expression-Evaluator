...
#if TEST_VARIABLE
	BOOST_AUTO_TEST_CASE(unitialized_single_variable) {
		...
		BOOST_CHECK(v->get_value() == nullptr);
	}
	/*BOOST_AUTO_TEST_CASE(unitialized_variable_expression) {
		try {
			auto result = RPNEvaluator().evaluate({ make<Variable>(), make<Negation>() });
			BOOST_FAIL("Failed to throw exception");
		}
		catch (std::exception& e) {
			BOOST_CHECK(strcmp(e.what(), "Error: variable not initialized") == 0);
		}
	}*/
#endif
